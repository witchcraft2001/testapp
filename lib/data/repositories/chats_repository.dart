...
abstract class ChatsRepository {
  Future<List<ChatFeedResponse>> getAllChats();

  Future<List<ChatFeedResponse>> getChatsWithUnreadMessages();

  Future<ChatInfoResponse> getChatInfo(String id);

  Future<List<ChatMessage>> getMessagesByChatId(String id);

  Future sendMessage(String chatId, String text);

  Future sendMenuItem(String chatId, String menuId, String value);

  Future sendForm(String chatId, FormRequest form);

  Future removeMessageById(String chatId, String clientMessageId);

  Subject<List<ChatFeedResponse>> get chatListSubject;

  Subject<String> get chatMessagesUpdatesSubject;

  Future resetUnreadMessages(String chatId);

  Future userLoggedOut();
...
class ChatsRepositoryImpl extends ChatsRepository {
  ...
  final ChatsDbRepository _chatsDbRepository;
  final UuidGenerator _uuid;
  final UserService _userService;
  final MessagesDbRepository _messagesDbRepository;
  final LogService _logService;

  final List<ChatInfoResponse> _chats = [];
  final Map<String, List<ChatMessage>> _messages = {};
  final BehaviorSubject<List<ChatFeedResponse>> _chatListSubject = BehaviorSubject();
  ...
  Future _handleChatActions(Map<String, dynamic> json) async {
  ...
  Future<List<ChatFeedResponse>> getAllChats() async {
    ...
    return await _getChatFeed();
  }

  @override
  Future<ChatInfoResponse> getChatInfo(String id) async {
    final chat = await _chatsDbRepository.getById(id);
    if (chat != null) {
      return chat.toChatInfoResponse();
    }
  ...
  @override
  Future sendMenuItem(String chatId, String menuId, String value) async {
    if (!_messages.keys.contains(chatId)) {
      _messages[chatId] = [];
    }
    final list = _messages[chatId];
  ...
  void _updateChatFeed() async {
    _chatListSubject.add(await _getChatFeed());
  }

  @override
  BehaviorSubject<List<ChatFeedResponse>> get chatListSubject => _chatListSubject;

  @override
  Subject<String> get chatMessagesUpdatesSubject => _chatMessagesUpdatesSubject;

  @override
  Future resetUnreadMessages(String chatId) async {
  ...
  @override
  Future<List<ChatFeedResponse>> getChatsWithUnreadMessages() async {
    await getAllChats();
    final result = (await _getChatFeed()).where((e) => e.unreadMessagesCount > 0).toList();

    return result;
  }

  @override
  Future removeMessageById(String chatId, String clientMessageId) async {
    if (_messages.keys.contains(chatId)) {
      final list = _messages[chatId];
  ...
  @override
  Future sendForm(String chatId, FormRequest form) async {
