...
class GreetingCardCubit extends Cubit<GreetingCardData> {
  ...
  Future<void> share(GlobalKey globalKey) async {
    ...
    await _shareUseCase(GreetingCardsUseCaseParams(path, state));
  }

  Future<NotificationSendingResult> send(GlobalKey globalKey) async {
    final path = await _create(globalKey);
    final result = await _sendByEmailUseCase(GreetingCardsUseCaseParams(path, state));
